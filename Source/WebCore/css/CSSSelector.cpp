...
{
    builder.append(WTF::interleave(list, appendPossiblyQuotedIdentifier, ", "_s));
}
    ...
    while (true) {
        ...
        if (selector->match() == Match::Id) {
            ...
            case PseudoClass::ActiveViewTransitionType:
                ...
                builder.append("("_s,
                    WTF::interleave(*selector->argumentList(), [&](auto& builder, auto& partName) {
            ...
            case PseudoElement::ViewTransitionNew:
                ...
                builder.append("::"_s, selector->serializingValue(), '(',
                    WTF::interleave(*selector->argumentList(), [&](auto& builder, auto& nameOrClass) {
